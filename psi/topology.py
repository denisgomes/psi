...
from openpipe.utils.bezier import bezier
...
class Curve(object):
    ...
    control points, curves of varing complexity can be approximated.  An arc
    or bend for example can be approximated using a quadratic bezier curve
    with three control points or a cubic bezier curve with four control
    points.  For the quadratic case, the control points will be the bend
    near, elbow and far points.
    ...
    Since accurary is very in CAD models, a rational bezier curve will be
    used as the mathematical model for a curve object.  As more and more
    edges are added a rational bezier curve will represent the underlying
    curve exactly.
    """
